...
test.describe('Kashif Profile Website', () => {
  ...
  test('has correct title and meta information', async ({ page }) => {
    await expect(page).toHaveTitle(/Kashif Jilani/);
  });
  ...
  test('hero section displays correctly', async ({ page }) => {
    await expect(page.locator('h1:has-text("Kashif Jilani")')).toBeVisible();
    await expect(page.locator('text=Senior .NET Engineer')).toBeVisible();
  ...
  test('about section content is present', async ({ page }) => {
    ...
    await expect(page.locator('text=Integrations & Reporting')).toBeVisible();
  });

  test('career timeline displays experience', async ({ page }) => {
    await page.locator('text=Experience').click();
    await expect(page.locator('h2:has-text("Career Journey")')).toBeVisible();
    await expect(page.locator('text=Macrosoft Inc.')).toBeVisible();
    await expect(page.locator('text=Khaleef Technologies')).toBeVisible();
    await expect(page.locator('text=Wizlinx')).toBeVisible();
    await expect(page.locator('text=Kohinoor Energy')).toBeVisible();
  });
  ...
  test('skills section shows technical skills', async ({ page }) => {
    ...
    await expect(page.locator('text=Reporting & Integrations')).toBeVisible();
  });
  ...
  test('portfolio section displays projects', async ({ page }) => {
    ...
    await expect(page.locator('text=My Cap')).toBeVisible();
    await expect(page.locator('text=Kashif Profile AI')).toBeVisible();
  });

  test('blog page loads (Sanity CMS)', async ({ page }) => {
    await page.goto('/blogs');
    await expect(page).toHaveTitle(/Blog \| Kashif Jilani/);
  ...
  test('AI chat interface is functional', async ({ page }) => {
    ...
    await expect(panel.locator('text=Kashif Jilani')).toBeVisible();
  ...
  test('contact section works', async ({ page }) => {
    ...
    await expect(page.locator('a[href="mailto:kashif-jilani@outlook.com"]').first()).toBeVisible();
